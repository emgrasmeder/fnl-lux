...
Every Love2D example must include `tests/startup-test.fnl`, which smoke-tests `love.load`, optional `love.update`, and `love.draw` using a headless Love mock (no Love2D binary required in CI). When adding a new example, register it in the `examples` list in root [`tasks/run-tests`](tasks/run-tests).
