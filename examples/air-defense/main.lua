local function add_fennel_path()
  ...
  if not love.filesystem.getInfo(base) then
    error("fennel rock missing — run `deps` and `deps --lua-version 5.1 --no-prompt` in examples/air-defense first")
  end
  ...
  error("fennel rock missing — run `deps --lua-version 5.1 --no-prompt` in examples/air-defense first")
end
