(import-macros
 {: deftest : testing : assert-is : assert-eq : assert-is}
...
(deftest create-game-world-test
  (testing "create-game-world"
    ...
    (let [game (world.create-game-world)]
      ...
      (assert-is (. game :ball-id))
      (assert-is (. game :crane-id))
      (assert-is (> (# (. game :brick-ids)) 0))
      (assert-is (<= (# (. game :brick-ids)) c.MAX-BRICKS)))))

(deftest brick-components-test
  (testing "brick entities have physics components"
    (math.randomseed 5)
    (let [game (world.create-game-world)
          id (. (. game :brick-ids) 1)
          comp (get-table-by-id (. game :world) id)]
      (assert-eq :brick (. comp.actor 1))
      (assert-is (. comp.position 1))
      (assert-is (. comp.velocity 1))
      (assert-is (. comp.rotation 1))
      (assert-is (. comp.angular-velocity 1)))))
